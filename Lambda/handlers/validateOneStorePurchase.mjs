...
import { verifyOneStorePurchase } from "../shared/storeVerification.mjs";
...
export const handler = async (event) => {
  ...
  if (!validate(body)) {
    logger.warn("Invalid One Store purchase validation request", { errors: validate.errors });
  ...
  try {
    ...
    if (!receipt.packageName || !receipt.productId || !receipt.purchaseToken) {
      return fail(ErrorCode.InvalidInput, "Invalid One Store receipt format");
    }

    await verifyOneStorePurchase({
    ...
    logger.error("ValidateOneStorePurchase failed", error);
